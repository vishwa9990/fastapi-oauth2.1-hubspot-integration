...
async def oauth2callback_hubspot_integration(code: str, state: str = ""):
    return await oauth2callback_hubspot(code, state)


@app.post('/integrations/hubspot/load')
async def get_hubspot_items(user_id: str = Form(...)):
    return await get_items_hubspot(user_id)
